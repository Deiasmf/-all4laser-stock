...
export type FichaProdutoDados = {
  ...
  descricaoModelo: string | null  // descrição standard do modelo (no idioma da ficha)
...
export async function gerarPdfFichaProduto(d: FichaProdutoDados): Promise<Blob> {
  ...
  // ── Descrição do modelo (standard, entra automaticamente) ──
  if (d.descricaoModelo && d.descricaoModelo.trim()) {
    tituloSeccao(t.descModelo)
    doc.setFont('helvetica', 'normal'); doc.setFontSize(10); doc.setTextColor(...NAVY)
    const linhas = doc.splitTextToSize(d.descricaoModelo.trim(), larguraUtil)
