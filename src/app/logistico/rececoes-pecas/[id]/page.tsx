...
import {
  obterRececao, listarItensRececao, alterarEstadoRececao, atualizarRececao,
  eliminarRececao,
...
// Link para a ficha do documento ligado (envio ou reparação), se existir.
    ...
    <main style={c.page}>
      <div style={c.cabecalho}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <BotaoPdf
            ...
            documento={() => ({
              ...
              seccoes: [
                ...
                {
                  titulo: eCliente ? 'Cliente' : 'Fornecedor',
                  linhas: [{ rotulo: 'Nome', valor: origemNome }],
                },
                {
                  titulo: 'Ligações',
                  linhas: [
                ...
                {
                  titulo: 'Notas',
                  linhas: [{ rotulo: 'Notas', valor: rececao.notas }],
                },
              ],
          ...
          />
          {isAdmin && (
            <button style={c.btnApagar} disabled={aTrabalhar} onClick={apagar}>🗑 Apagar</button>
          )}
      ...
      <section style={c.card}>
        ...
        <Linha
          rotulo="Documento ligado"
          valor={rececao.referencia_numero
            ? `${rececao.referencia_numero}${rececao.referencia_tipo && rececao.referencia_tipo !== 'manual' ? ` · ${REFERENCIA_TIPO_LABEL[rececao.referencia_tipo]}` : ''}`
            : null}
        />
      ...
      {rececao.estado === 'conferido' && (
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <button style={c.btnGhost} disabled={aTrabalhar} onClick={() => mudar('aberto')}>Reabrir (por conferir)</button>
        </div>
      )}
