...
type Mapa = { modelo_grupo: string; modelo_label: string }
...
export default function AgendaCalendariosPage() {
    ...
    ;(async () => {
      ...
      try {
        const [rCal, rMap] = await Promise.all([
          ...
          supabase.from('calendarios_aluguer').select('id, modelo_grupo, modelo_label'),
        ])
        if (!ativo) return
        setCarregando(false)
        ...
        for (const row of (rMap.data ?? []) as { id: string; modelo_grupo: string; modelo_label: string }[]) {
          m[row.id] = { modelo_grupo: row.modelo_grupo, modelo_label: row.modelo_label }
        }
        setMapas(m)
      } catch {
  ...
  async function mapear(cal: Calendario, grupo: string) {
    if (!grupo) {
      // "— ignorar —": remove o mapeamento
    ...
    setMapas((p) => ({ ...p, [cal.id]: { modelo_grupo: grupo, modelo_label: label } }))
    const { error } = await supabase.from('calendarios_aluguer').upsert({
      id: cal.id, nome: cal.nome, modelo_grupo: grupo, modelo_label: label, updated_at: new Date().toISOString(),
    })
  ...
  return (
    <main style={c.page}>
      ...
      <p style={c.sub}>
        Associa cada calendário de aluguer a um modelo (para calcular o valor das marcações).
      ...
      {carregando ? (
        ...
        <>
              ...
              return (
                <div key={cal.id} style={c.linha}>
                  ...
                  <select
                    ...
                    onChange={(e) => mapear(cal, e.target.value)}
          ...
          <p style={c.sub}>
            A seguir (Fase 3): ler as marcações destes calendários e calcular a previsão de receita por mês.
          </p>
        </>
      )}
    </main>
...
const c: Record<string, React.CSSProperties> = {
  ...
  linha: { display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 10, padding: '10px 8px', fontSize: 14, borderBottom: '1px solid #f2f2f2', alignItems: 'center', minWidth: 520 },
